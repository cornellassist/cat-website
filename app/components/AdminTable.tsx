...
export function AdminTable({ data, update }: { data?: any[]; update?: (row: number, field: string, value: string) => void; }) {
  const path = usePathname();
  const [modalOpen, setModalOpen] = useState<boolean>(false);
  const [editCell, setEditCell] = useState<{ row: number; col: string } | null>(null);
  const [editedValue, setEditedValue] = useState<string>("");
  const [tableData, setTableData] = useState(data);
  const [pendingChanges, setPendingChanges] = useState<{ row: number; field: string; value: string }[]>([]);
  ...
  const commitEdit = () => {
    if (editCell) {
      ...
      setPendingChanges(prev => [
        ...prev.filter(c => !(c.row === editCell.row && c.field === editCell.col)), // dedupe
  ...
  const saveChanges = async () => {
    await Promise.all(
      pendingChanges.map(({ row, field, value }) => update?.(row, field, value))
    );
    setPendingChanges([]);
  };
  ...
  return (
    <div className="mx-10 flex flex-col gap-4">
                ...
                {colHeaders.map((col) => {
                  ...
                  return (
                    <td
                      ...
                      onClick={() => !isEditing && startEditing(rowIndex, col, cellValue)}
                    >
                      {isEditing ? (
                        <input
                          className="w-full border border-blue-400 rounded px-1.5 py-0.5 text-sm outline-none 
                          focus:ring-2 focus:ring-blue-300 bg-white" autoFocus value={editedValue} onChange={(e) => setEditedValue(e.target.value)} onKeyDown={handleEnter} onBlur={commitEdit}
                        />
                      ) : (
                        <span className="block cursor-pointer truncate">
                          {cellValue ?? <span className="text-theme-dk-red">None</span>}
                        </span>
                      )}
                    </td>
                  );
      ...
      <div className="flex justify-end">
        <button className="bg-theme-red text-white py-2 px-4 rounded-lg hover:bg-theme-red/80 transition-colors duration-200 disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer" disabled={pendingChanges.length === 0} onClick={saveChanges}>
          Save Changes
        </button>
      </div>
    </div>
